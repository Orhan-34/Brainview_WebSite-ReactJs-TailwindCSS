...
const Header = () => {
	...
	return (
		<div
			className={`fixed top-0 z-50 w-full left-0
    border-b border-n-6 lg: bg-n-8 lg:backdrop-blur-sm ${openNavigation ? "bg-n-8" : "bg-n-8/90 backdrop-blur-sm"}`}
		>
						...
						{navigation.map((item) => (
							<a
								...
								className={`block relative font-code text-2xl uppercase text-n-1 
                                    ...
                                    ${item.url === pathname.hash ? "lg:text-n-1 z-2" : "lg:text-n-1/50"} lg:leading-5 lg:hover:text-color-1 xl: px-12`}
							>
								{item.title}
							</a>
						))}
